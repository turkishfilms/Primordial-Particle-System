class Agent {
  ...
  go() {
    ...
    for (let n of this.neighbors) {
      var no = atan2(n.y - this.y, n.x - this.x)
      if (no < 0) no += TWO_PI
      if (no > PI + this.o || no < this.o) ln.push(n)
      else if (no < PI + this.o || no > this.o) rn.push(n)
    }

    var b = 0;
    // which way to turn
    b = ln.length >= rn.length ? -(this.b * this.neighbors.length) : this.b * this.neighbors.length
